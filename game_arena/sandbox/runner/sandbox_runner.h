...
// workspace, the timeout handling and the kill all live in
...
struct SandboxRunnerConfig {
  ...
  // The tree each run gets a copy of. Never mounted, so the host checkout
  // stays untouched and the daemon never has to see it.
