...
TEST_F(CandidateStoreTest, AcceptsAPatchDirectly) {
  ...
  request.set_patch(
      "diff --git a/problem/lib/core.h b/problem/lib/core.h\n"
      "--- a/problem/lib/core.h\n"
      "+++ b/problem/lib/core.h\n"
      "@@ -1,2 +1,2 @@\n"
      "-slow\n"
      "+fast\n");
  std::string error;
...
TEST_F(CandidateStoreTest, HoldsPatchesToTheProblemsPathPolicy) {
  ...
  // Outside allow_paths.
  EXPECT_FALSE(
      store.Validate(patch_touching("problem/tools/bench.cc"), &error));
  ...
  EXPECT_FALSE(store.Validate(patch_touching("problem/lib/BUILD"), &error));
...
TEST_F(CandidateStoreTest, SynthesizedPatchAppliesWithGit) {
  ...
  const std::filesystem::path root =
      repo / "solutions" / candidate->candidate_id();
...
TEST_F(CandidateStoreTest, RejectsDependenciesOutsideTheAllowlist) {
  ...
  request.add_extra_deps("//problem/strategies:risk_proposer");
  request.add_extra_deps("//problem/lib:mcts");
